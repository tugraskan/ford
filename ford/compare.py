...
def compare_metadata(old_metadata: Dict[str, Any], 
                     new_metadata: Dict[str, Any]) -> ComparisonResult:
    """
    ...
    result.modified_modules = old_modules & new_modules  # Simplified - could be enhanced
...
def format_report(result: ComparisonResult, verbose: bool = False) -> str:
    ...
    lines.append("=" * 80)
    total_changes = (
        len(result.new_modules) + len(result.removed_modules) +
        len(result.new_procedures) + len(result.removed_procedures) +
        len(result.new_types) + len(result.removed_types) +
        len(result.new_files) + len(result.removed_files)
    )
    lines.append(f"Total changes detected: {total_changes}")
    lines.append(f"  Modules: +{len(result.new_modules)} -{len(result.removed_modules)}")
    lines.append(f"  Procedures: +{len(result.new_procedures)} -{len(result.removed_procedures)}")
...
def compare_commits(old_json: Path, new_json: Path, 
...
Examples:
  ...
  ford-compare old.json new.json --verbose
    ...
    parser.add_argument('old_metadata', type=Path,
                       help='Path to the older metadata JSON file')
    parser.add_argument('new_metadata', type=Path,
                       help='Path to the newer metadata JSON file')
    ...
    try:
        compare_commits(args.old_metadata, args.new_metadata, 
